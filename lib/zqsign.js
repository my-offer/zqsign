...
class ZQSign {
    ...
    doRequest(options, formData) {
        ...
        const signture = this.signer.signQuery(formData)
        const isVerified = this.signer.verifyQuery(formData, signture)
        formData.sign_val = signture
        assert.ok(isVerified, 'isVerified false')
        const shallow = lodash.clone(formData)
